...
//        $detail = explode("<div class=\"cif-article", $detail);
    ...
    {
        $url = "https://www.kchuhai.com/report/key%E7%BD%91%E7%BA%A2_pg1";
        $data = $this->GetHttp($url);
        $pos1 = strpos($data, "<div class=\"layui-tab-item layui-show\"");
        $pos2 = strpos($data, "<div class=\"kch-rightBox\"");
        $detail = substr($data, $pos1 , $pos2 - $pos1-20 );
    ...
    {
        $reg3 = "/<div class=\"font-16 text-333 lineHeight flex flex-column mb-3 kch-detailBox position-relative overflow-hidden\"([\S\s]+?)<\/div><div class=\"flex justify-start flex-wrap/";
        ...
        $desc2[0][0] = preg_replace('#width="[^"]*"#i', '', $desc2[0][0]);
        $desc2[0][0] = preg_replace('#height="[^"]*"#i', '', $desc2[0][0]);
